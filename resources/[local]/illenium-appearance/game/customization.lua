...
local cachedAces = nil
local function getPlayerAces()
    if not Config.Aces or #Config.Aces == 0 then return {} end
    if cachedAces then return cachedAces end
    local success, result = pcall(function()
        return lib.callback.await("illenium-appearance:server:GetPlayerAces", false)
    end)
...
local function allowedForPlayer(item, allowedAces)
    ...
    local citizenId = client.citizenid or ""
    return (item.jobs and listContains(item.jobs, jobName)) or (item.gangs and listContains(item.gangs, gangName)) or (item.aces and listContainsAny(item.aces, allowedAces) or (item.citizenids and listContains(item.citizenids, citizenId)))
end
...
function client.startPlayerCustomization(cb, conf)
    ...
    local baseConfig = (type(GetDefaultConfig) == "function" and GetDefaultConfig()) or {
        ped = true,
        headBlend = true,
        faceFeatures = true,
        headOverlays = true,
    ...
    if conf then
        ...
        if conf.componentConfig then
            for k, v in pairs(conf.componentConfig) do
                baseConfig.componentConfig[k] = v
            end
        end
        if conf.propConfig then
            for k, v in pairs(conf.propConfig) do
                baseConfig.propConfig[k] = v
            end
        end
    end
    config = baseConfig
