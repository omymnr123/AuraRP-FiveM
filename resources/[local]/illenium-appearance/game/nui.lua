...
RegisterNUICallback("appearance_save", function(appearance, cb)
    ...
    if appearance then
        ...
        pcall(function() client.wearClothes(appearance, "head") end)
        pcall(function() client.wearClothes(appearance, "body") end)
        pcall(function() client.wearClothes(appearance, "bottom") end)
    end
...
RegisterNUICallback("appearance_rotate_heading", function(data, cb)
    cb(1)
    if data and data.delta then
        local currentHeading = GetEntityHeading(cache.ped)
        SetEntityHeading(cache.ped, (currentHeading + data.delta) % 360.0)
    end
end)
