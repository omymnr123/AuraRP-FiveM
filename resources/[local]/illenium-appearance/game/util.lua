...
local function computePedModelsByHash()
    pedModelsByHash[`mp_m_freemode_01`] = "mp_m_freemode_01"
    pedModelsByHash[`mp_f_freemode_01`] = "mp_f_freemode_01"
...
local function setPedHeadOverlays(ped, headOverlays)
    if headOverlays then
        for k, v in pairs(constants.HEAD_OVERLAYS) do
            ...
            if headOverlay and headOverlay.style then
                SetPedHeadOverlay(ped, k-1, headOverlay.style, tofloat(headOverlay.opacity or 0))
