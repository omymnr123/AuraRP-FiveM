    ...
    CreateThread(function()
        pcall(function()
            ...
            if HasAnimDictLoaded(dict) then
                TaskPlayAnim(ped, dict, anim, 8.0, 3.0, duration or 400, 51, 0.0, false, false, false)
            end
        end)
    end)
end
...
local function EnsureBaseAppearance()
    local ped = cache.ped or PlayerPedId()
    if not cachedBaseAppearance then
        if exports['illenium-appearance'] then
            pcall(function()
                cachedBaseAppearance = exports['illenium-appearance']:getPedAppearance(ped)
            end)
        end
        if not cachedBaseAppearance or not cachedBaseAppearance.components then
            -- Fallback snapshot nativo directo de todos los componentes y props
            cachedBaseAppearance = {
                components = {},
                props = {}
            }
            for i = 0, 11 do
                table.insert(cachedBaseAppearance.components, {
                    component_id = i,
            ...
            for i = 0, 7 do
                table.insert(cachedBaseAppearance.props, {
                    prop_id = i,
...
local function GetBaseComponent(compIndex)
    ...
    if cachedBaseAppearance and cachedBaseAppearance.components then
        for _, comp in pairs(cachedBaseAppearance.components) do
...
local function GetBaseProp(propIndex)
    ...
    if cachedBaseAppearance and cachedBaseAppearance.props then
        for _, prop in pairs(cachedBaseAppearance.props) do
...
local function ToggleClothingPiece(pieceType)
    if not pieceType or type(pieceType) ~= 'string' then
        return false, clothingToggles
    end
    ...
    pcall(function()
        if pieceType == 'hat' then
            PlayClothingAnimation('mp_masks@standard_car@ds@', 'put_on_mask', 400)
        ...
        elseif pieceType == 'mask' then
            PlayClothingAnimation('mp_masks@standard_car@ds@', 'put_on_mask', 500)
            if newWornState then
        ...
        elseif pieceType == 'glasses' then
            PlayClothingAnimation('clothingspecs', 'take_off', 450)
            if newWornState then
        ...
        elseif pieceType == 'ear' then
            PlayClothingAnimation('mp_cp_stolen_tut', 'b_adjust_earpiece', 400)
            if newWornState then
        ...
        elseif pieceType == 'chain' then
            PlayClothingAnimation('clothingtie', 'try_tie_negative_a', 500)
            if newWornState then
        ...
        elseif pieceType == 'gloves' then
            PlayClothingAnimation('nmt_3_rcm-10', 'cs_nigel_dual-10', 400)
            if newWornState then
                local comp3, tex3 = GetBaseComponent(3)
                SetPedComponentVariation(ped, 3, comp3 or 15, tex3 or 0, 0)
            else
        ...
        elseif pieceType == 'pants' then
            PlayClothingAnimation('re@construction', 'out_of_breath', 550)
            if newWornState then
        ...
        elseif pieceType == 'shoes' then
            PlayClothingAnimation('random@domestic', 'pickup_low', 500)
            if newWornState then
        ...
        elseif pieceType == 'bag' then
            PlayClothingAnimation('anim@heists@ornate_bank@grab_cash', 'intro', 500)
            if newWornState then
...
-- ============================================================================
-- EXPORTS Y MÉTODOS INTERNOS CLIENT
-- ============================================================================

client.ToggleClothing = ToggleClothingPiece
client.GetClothingState = function() return clothingToggles end
client.SetClothingState = function(toggles) clothingToggles = toggles or clothingToggles end
client.SetBaseAppearance = function(appearance) cachedBaseAppearance = appearance end

exports('ToggleClothing', function(arg1, arg2)
    local pieceType = arg1
    if type(arg1) == 'table' and type(arg2) == 'string' then
        pieceType = arg2
    elseif type(arg1) == 'table' then
        pieceType = arg1.type or arg1[1]
    end
    return ToggleClothingPiece(pieceType)
end)
