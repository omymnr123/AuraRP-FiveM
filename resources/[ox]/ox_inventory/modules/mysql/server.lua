...
Citizen.CreateThreadNow(function()
    ...
    elseif shared.framework == 'qbx' then
        ...
        vehicleColumn = 'id'
    elseif shared.framework == 'aura' then
    ...
    if vehicleTable then
        local hasVehicleTable = pcall(MySQL.scalar.await, ('SELECT 1 FROM `%s` LIMIT 1'):format(vehicleTable))
        if not hasVehicleTable then
            MySQL.query.await(([[CREATE TABLE IF NOT EXISTS `%s` (
                ...
                `trunk` LONGTEXT NULL
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;]]):format(vehicleTable))
        end
    end
