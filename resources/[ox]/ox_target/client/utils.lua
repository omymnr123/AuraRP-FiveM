...
---@return vector3 surfaceNormal
---@return number materialHash
function utils.raycastFromScreen(screenX, screenY, flag, distance)
    local coords, normal = GetWorldCoordFromScreenCoord(screenX, screenY)
    local destination = coords + normal * (distance or 20.0)
    return lib.raycast.fromCoords(coords, destination, flag or 511, 4)
end
